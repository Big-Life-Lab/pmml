...
convert_model_export_to_pmml <-
  function(model_parameters_folder_path, model_export_file_path, database_name, custom_function_files = NULL) {
    ...
    variable_details_path <-
      model_export[model_export$fileType == pkg.env$ModelExportCSV.variable_details, pkg.env$ModelStepsCSV.filePath]
    variable_details_path <-
    ...
    model_steps <-
      read.csv(model_steps_path,
               ...
               stringsAsFactors = FALSE)
    detected_steps <-
      unique(model_steps[[pkg.env$ModelStepsCSV.step]])
    step_list <- list()
    
    for (working_step in detected_steps) {
      current_rows <-
        model_steps[model_steps[[pkg.env$ModelStepsCSV.step]] == working_step, c(pkg.env$ModelStepsCSV.fileType,
                                                                                     pkg.env$ModelStepsCSV.filePath)]
      step_list[[working_step]] <-
        list(fileType = current_rows[[pkg.env$ModelStepsCSV.fileType]], filePath = current_rows[[pkg.env$ModelStepsCSV.filePath]])
    }
      ...
      for (single_step in 1:length(step_list)) {
        ...
        if (length(step_list[[single_step]][[1]]) > 1) {
          for (sub_step in 1:length(step_list[[single_step]][[1]])) {
            step_list[[single_step]][[pkg.env$ModelInternal.file]][[sub_step]] <-
              read_step_file(model_export_file_path, step_list[[single_step]][[pkg.env$ModelStepsCSV.filePath]][[sub_step]])
          }
        } else{
          step_list[[single_step]][[pkg.env$ModelInternal.file]] <-
            read_step_file(model_export_file_path, step_list[[single_step]][[pkg.env$ModelStepsCSV.filePath]])
        }
      }
    }
    ...
    doc$namespaceDefinitions <- c(
      pkg.env$PMML.Node.Attributes.Value.xmlns,
      xsi = pkg.env$PMML.Node.Attributes.Value.xsi
    )
    doc$attrs <- c(version = pkg.env$PMML.Node.Attributes.Value.PMML.Version)
    
    header <- XML::xmlNode(pkg.env$PMML.Node.Header)
    header <-
    ...
    if('fine-and-gray' %in% model_steps$step) {
      ...
      min_time <-
        min(as.character(variable_details_time_rows[[pkg.env$argument.recEnd]]))
      working_pmml[[pkg.env$PMML.Node.DataDictionary]] <-
        XML::addChildren(
          working_pmml[[pkg.env$PMML.Node.DataDictionary]],
          XML::xmlNode(
            pkg.env$PMML.Node.DataField,
            attrs = c(
              name = pkg.env$variables.Time,
              displayName = "time of predicted probability",
              optype = pkg.env$PMML.Node.Attributes.Value.optype.cont,
              dataType = pkg.env$PMML.Node.Attributes.Value.dataType.float
            ),
            XML::xmlNode(
              pkg.env$PMML.Node.Extension,
              attrs = c(
                name = pkg.env$PMML.Extension.names.units,
                value = variable_details_time_rows[1, ][[pkg.env$argument.Units]]
              )
            ),
            XML::xmlNode(
              pkg.env$PMML.Node.Extension,
              attrs = c(
                name = pkg.env$PMML.Extension.names.variableStartLabel,
                value = ""
              )
            ),
            XML::xmlNode(
              pkg.env$PMML.Node.Interval,
              attrs = c(
      ...
      for (end_variable in all_unique_vars) {
        working_row <-
          variable_details[variable_details[[pkg.env$argument.Variables]] == end_variable,]
        ...
        if(!is_derived_var(working_row)) {
          ...
          start_var_name <- get_start_var_name(working_row, database_name)
          if(start_var_name %in% variable_details[[pkg.env$argument.Variables]] == FALSE) {
            all_start_vars <-
              append(
                all_start_vars,
                start_var_name
    ...
    for (step_name in names(step_list)) {
      ...
      if (length(working_step[[pkg.env$ModelStepsCSV.fileType]]) > 1) {
        for (sub_step_index in 1:length(working_step[[pkg.env$ModelStepsCSV.fileType]])) {
          working_pmml <-
            convert_step(
              current_file_type = working_step[[pkg.env$ModelStepsCSV.fileType]][[sub_step_index]],
              step_name,
              current_file = working_step[[pkg.env$ModelInternal.file]][[sub_step_index]],
              all_start_vars,
              max_time,
              working_pmml,
        ...
        working_pmml <-
          convert_step(
            current_file_type = working_step[[pkg.env$ModelStepsCSV.fileType]],
            step_name,
            current_file = working_step[[pkg.env$ModelInternal.file]],
            all_start_vars,
            max_time,
            working_pmml,
            model_parameters_folder_path
...
create_dummy_nodes <-
  function(orig_variable,
    ...
    for (vector_index in 1:length(orig_variable)) {
      ...
      current_main_node <-
        XML::xmlNode(
          pkg.env$PMML.Node.DerivedField,
          attrs = c(
            name = dummy_variable[[vector_index]],
            optype = pkg.env$PMML.Node.Attributes.Value.optype.cat,
            dataType = pkg.env$PMML.Node.Attributes.Value.dataType.float
          )
        )
      ...
      current_apply_node_parent <-
        XML::xmlNode(
          pkg.env$PMML.Node.Apply,
          attrs = c('function' = pkg.env$PMML.Node.Attributes.Value.function.if)
        )
      current_apply_node <-
        XML::xmlNode(
          pkg.env$PMML.Node.Apply,
          attrs = c('function' = pkg.env$PMML.Node.Attributes.Value.function.equal)
        )
      ...
      current_field_node <-
        XML::xmlNode(pkg.env$PMML.Node.FieldRef,
                     attrs = c(field = orig_variable[[vector_index]]))
      current_const_apply_node <-
        XML::xmlNode(
          pkg.env$PMML.Node.Constant,
          attrs = c(dataType = pkg.env$PMML.Node.Attributes.Value.dataType.float),
      ...
      PMML[[pkg.env$PMML.Node.TransformationDictionary]] <-
        XML::addChildren(PMML[[pkg.env$PMML.Node.TransformationDictionary]], current_main_node)
    }
    return(PMML)
  }
  ...
  function(orig_variable,
    ...
    for (vector_index in 1:length(orig_variable)) {
      ...
      current_apply_node <-
        XML::xmlNode(
          pkg.env$PMML.Node.Apply,
          attrs = c('function' = pkg.env$PMML.Node.Attributes.Value.function.minus)
        )
      current_field_node <-
        XML::xmlNode(pkg.env$PMML.Node.FieldRef,
                     attrs = c(field = orig_variable[[vector_index]]))
      ...
      PMML[[pkg.env$PMML.Node.TransformationDictionary]] <-
        XML::addChildren(PMML[[pkg.env$PMML.Node.TransformationDictionary]], current_node)
    }
...
create_rcs_nodes <- function(variable, rcs_variables, knots, PMML) {
  ...
  for (vector_index in 1:length(variable)) {
    ...
    current_knots_raw <-
      strsplit(knots[[vector_index]], pkg.env$variables.splitValue)[[1]]
    current_knots <- paste(current_knots_raw, collapse = " ")
    current_array_node <-
      XML::xmlNode(
        pkg.env$PMML.Node.Array,
        attrs = c(
          n = length(current_knots_raw),
          type = pkg.env$PMML.Node.Attributes.Value.Type.float
        ),
        current_knots
      )
    ...
    current_first_field_node <-
      XML::xmlNode(pkg.env$PMML.Node.FieldRef, attrs = c(field = variable[[vector_index]]))
    current_first_node <-
      XML::addChildren(
        current_first_node,
        current_first_field_node
    ...
    PMML[[pkg.env$PMML.Node.TransformationDictionary]] <-
      XML::addChildren(PMML[[pkg.env$PMML.Node.TransformationDictionary]], current_first_node)
    ...
    for (rcs_variable_index in 2:length(variable_list)) {
      ...
      current_apply_node <-
        XML::xmlNode(
          pkg.env$PMML.Node.Apply,
          attrs = c('function' = pkg.env$PMML.Node.Attributes.Value.function.rcs)
        )
      current_field_node <-
        XML::xmlNode(pkg.env$PMML.Node.FieldRef,
    ...
    for (vector_index in 1:length(interaction_variable)) {
      variable_list <-
        strsplit(interacting_variables[[vector_index]],
                 pkg.env$variables.splitValue)[[1]]
      ...
      current_apply_node <-
        XML::xmlNode(
          pkg.env$PMML.Node.Apply,
          attrs = c(
            'function' = pkg.env$PMML.Node.Attributes.Value.function.multiplication
          )
        )
...
create_beta_coefficient_nodes <-
  function(variable,
    ...
    current_param_list_node <-
      XML::xmlNode(pkg.env$PMML.Node.ParameterList)
    # Create a ParameterList child node Paramater with name set to p0 and label to Intercept
    current_param_list_node <-
      XML::addChildren(
        current_param_list_node,
        XML::xmlNode(
          pkg.env$PMML.Node.Parameter,
          attrs = c(
            name = pkg.env$PMML.Node.Attributes.Value.startVar,
            label = pkg.env$PMML.Node.Attributes.Value.label.Intercept
          )
        )
      )
    current_param_matrix_node <-
      XML::xmlNode(pkg.env$PMML.Node.ParamMatrix)
    ...
    current_param_matrix_node <-
      XML::addChildren(current_param_matrix_node,
                       XML::xmlNode(
                         pkg.env$PMML.Node.PCell,
                         attrs = c(
                           parameterName = pkg.env$PMML.Node.Attributes.Value.startVar,
                           beta = "0"
                         )
                       ))
    current_factor_list_node <-
      XML::xmlNode(pkg.env$PMML.Node.FactorList)
    current_covariate_list_node <-
      XML::xmlNode(pkg.env$PMML.Node.CovariateList)
    current_pp_matrix_node <-
      XML::xmlNode(pkg.env$PMML.Node.PPMatrix)
    ...
    for (variable_index in 1:length(variable)) {
      ...
      current_param_list_node <-
        XML::addChildren(
          ...
          XML::xmlNode(
            ...
            attrs = c(
              name = paste0(
                pkg.env$PMML.Node.Attributes.Value.Var.incrementVar,
                variable_index
              ),
      ...
      current_predictor <-
        XML::xmlNode(pkg.env$PMML.Node.Predictor, attrs = c(name = variable[[variable_index]]))
    ...
    current_mining_schema_node <-
      XML::xmlNode(pkg.env$PMML.Node.MiningSchema)
    current_mining_schema_node <-
      XML::addChildren(
        current_mining_schema_node,
        XML::xmlNode(
    ...
    if (is.null(PMML[[pkg.env$PMML.Node.GeneralRegressionModel]])) {
      ...
      PMML <-
        XML::addChildren(PMML,
                         XML::xmlNode(
                           pkg.env$PMML.Node.GeneralRegressionModel,
                           attrs = c(
                             modelType = pkg.env$PMML.Node.Attributes.Value.modelType.CoxRegression,
                             functionName = pkg.env$PMML.Node.Attributes.Value.modelFunction.regression,
                             endTimeVariable = pkg.env$variables.Time
                           )
                         ))
    }
    PMML[[pkg.env$PMML.Node.GeneralRegressionModel]] <-
      XML::addChildren(
        PMML[[pkg.env$PMML.Node.GeneralRegressionModel]],
        current_mining_schema_node,
        current_param_list_node,
...
create_baseline_hazards_nodes <-
  function(time, baseline_hazard, max_time, PMML) {
    ...
    current_base_cum_hazard_table_node <-
      XML::xmlNode(pkg.env$PMML.Node.BaseCumHazardTables,
    ...
    for (vector_index in 1:length(time)) {
      ...
      current_base_cum_hazard_table_node <-
        XML::addChildren(
          current_base_cum_hazard_table_node,
          XML::xmlNode(
            pkg.env$PMML.Node.BaselineCell,
    ...
    PMML[[pkg.env$PMML.Node.GeneralRegressionModel]] <-
      XML::addChildren(PMML[[pkg.env$PMML.Node.GeneralRegressionModel]], current_base_cum_hazard_table_node)
...
create_simple_model_nodes <- 
  function(simple_model, PMML) {
    output_variable_name <- simple_model[simple_model[[pkg.env$simple_model_name_column]] == pkg.env$simple_model_output_variable_entry, ][[pkg.env$simple_model_value_column]]
    output_node <- XML::xmlNode(pkg.env$PMML.OutputNode)
    
    output_field_attrs <- c()
    output_field_attrs[[pkg.env$PMML.OutputFieldNode.attrs.name]] <- 
      output_variable_name
    output_field <- XML::xmlNode(
      pkg.env$PMML.OutputFieldNode, 
      attrs = output_field_attrs
    )
    output_node <- XML::append.xmlNode(output_node, output_field)
    PMML <- XML::append.xmlNode(PMML, output_node)
    PMML <- XML::append.xmlNode(PMML, XML::xmlNode(pkg.env$PMML.SimpleModelNode))
...
cont_and_cat_converter <- function(switch_value) {
  ...
  switch (
    ...
    "cont" = {
      optype <- pkg.env$PMML.Node.Attributes.Value.optype.cont
      data_type <-
        pkg.env$PMML.Node.Attributes.Value.dataType.float
    },
    "cat" = {
      optype = pkg.env$PMML.Node.Attributes.Value.optype.cat
      data_type <-
        pkg.env$PMML.Node.Attributes.Value.dataType.string
    }
  )
